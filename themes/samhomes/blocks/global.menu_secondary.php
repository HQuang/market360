...
if (! nv_function_exists('nv_menu_secondary')) {
    /**
     ...
     *
     */
    function nv_menu_secondary_check_current($url, $type = 0)
    {
     ...
     *
     */
    function nv_menu_secondary($block_config)
    {
        global $nv_Cache, $global_config, $site_mods, $module_info, $module_name, $module_file, $module_data, $lang_global, $catid, $home;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.menu_secondary.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.menu_secondary.tpl')) {
        ...
        $xtpl = new XTemplate('global.menu_secondary.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
        ...
        if (!empty($array_menu)) {
            foreach ($array_menu[0] as $id => $item) {
                if($home == 1){
                    $item['home']= 'class = "active"';
                }
                $classcurrent = array();
                $submenu_active = array();
                if (isset($array_menu[$id])) {
                    $classcurrent[] = 'dropdown';
                    $submenu = nv_get_primary_submenu($id, $array_menu, $submenu_active, $block_theme);
                    $xtpl->assign('SUB', $submenu);
                    $xtpl->parse('main.top_menu.sub');
                    $xtpl->parse('main.top_menu.has_sub');
                }
                if (nv_menu_secondary_check_current($item['link'], $item['active_type'])) {
                    $classcurrent[] = 'active';
     ...
     * @return string
     */
    function nv_get_primary_submenu($id, $array_menu, $submenu_active, $block_theme)
    {
        $xtpl = new XTemplate('global.menu_secondary.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');

        if (! empty($array_menu[$id])) {
            foreach ($array_menu[$id] as $sid => $smenu) {
                if (nv_menu_secondary_check_current($smenu['link'], $smenu['active_type'])) {
                    $submenu_active[] = $id;
                }
                $submenu = '';
                if (isset($array_menu[$sid])) {
                    $submenu = nv_get_primary_submenu($sid, $array_menu, $submenu_active, $block_theme);
                    $xtpl->assign('SUB', $submenu);
...
if (defined('NV_SYSTEM')) {
    $content = nv_menu_secondary($block_config);
}