...
if (! nv_function_exists('nv_menu_three')) {
    /**
     * nv_menu_three()
     *
     ...
     *
     */
    function nv_menu_three($block_config)
    {
        global $nv_Cache, $global_config, $site_mods, $module_info, $module_name, $module_file, $module_data, $lang_global, $catid, $home;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.menu_three.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.menu_three.tpl')) {
        ...
        $array_menu = array();
        $sql = 'SELECT id, parentid, title, link, icon, image, note, subitem, groups_view, module_name, op, target, css, active_type FROM ' . NV_PREFIXLANG . '_menu_rows WHERE status=1 AND mid = ' . $block_config['menuid'] . ' ORDER BY weight ASC';
        ...
        $xtpl = new XTemplate('global.menu_three.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
...
if (defined('NV_SYSTEM')) {
    $content = nv_menu_three($block_config);
}