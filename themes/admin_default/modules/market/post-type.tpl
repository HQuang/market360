    ...
    <div class="panel panel-default">
        <div class="panel-body">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-19 col-md-20">
                    <input class="form-control price" type="text" name="price" value="{ROW.price}" required="required" />
                </div>
            </div>
            <div class="form-group">
...
//<![CDATA[
	...
	function nv_change_weight(id) {
		...
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=post-type&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
			var r_split = res.split('_');
			if (r_split[0] != 'OK') {
				alert(nv_is_change_act_confirm[2]);
			}
			window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=post-type';
			return;
		});
		return;
	}
	...
	function nv_change_status(id) {
		...
		if (confirm(nv_is_change_act_confirm[0])) {
			...
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=post-type&nocache=' + new Date().getTime(), 'change_status=1&id='+id, function(res) {
				var r_split = res.split('_');
				if (r_split[0] != 'OK') {
					alert(nv_is_change_act_confirm[2]);
				}
