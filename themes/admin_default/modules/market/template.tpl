...
<div class="row" style="margin-bottom: 5px">
    <div class="col-xs-24">
        ...
        <div class="btn-group pull-right">
            <button type="button" class="btn btn-primary btn-xs" id="field">{LANG.field_id}</button>
            <button type="button" class="btn btn-primary  btn-xs dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                <span class="caret"></span> <span class="sr-only"></span>
            </button>
            <ul class="dropdown-menu" role="menu">
                <li><a href="{FIELD_ADD}">{LANG.captionform_add}</a></li>
            </ul>
        </div>
    </div>
</div>
...
<table class="table table-striped table-bordered table-hover">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><select id="change_weight_{id}" onchange="nv_chang_weight('{id}');" class="form-control input-sm">
                    <!-- BEGIN: weight -->
                    <option value="{WEIGHT.w}"{WEIGHT.selected}>{WEIGHT.w}</option>
                    <!-- END: weight -->
            </select></td>
            <td><a href="{link_field}">{title}</a></td>
