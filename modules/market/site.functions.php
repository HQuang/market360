...
{
    $string = strtolower(nv_EncString($string));
    if (preg_match('/([0-9\.\,]+) ([(ti|ty|trieu)]+) ([0-9\.\,]+)/', $string, $m)) {
        $m[1] = preg_replace('/[^0-9]/', '', $m[1]);
        $m[1] = $m[1];
        if ($m[2] == 'ti' || $m[2] == 'ty') {
            $return = $m[1] * 1000000000;
        }
        if (isset($m[3])) {
            $return = $return + ($m[3] * 1000000);
        }
        return $return;
    } elseif (preg_match('/^([0-9\,\.]+) ([(ti|ty|trieu)]+)/', $string, $m)) {
        $m[1] = preg_replace('/[^0-9\,\.]/', '', $m[1]);
        if ($m[2] == 'ti' || $m[2] == 'ty') {
            return $m[1] * 1000000000;
        } else {
    ...
    } elseif (preg_match('/([0-9]+) (trieu|ty)/', $string, $m)) {
        if ($m[2] == 'ti' || $m[2] == 'ty') {
            return $m[1] * 1000000000;
        } else {
            return $m[1] * 1000000;
        }
    } elseif (preg_match('/([0-9\.\,]+)/', $string, $m)) {
