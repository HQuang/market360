 ...
 *
 */
function nv_theme_market_main($array_data, $viewtype, $page, $sort = 0)
{
    global $global_config, $module_name, $module_file, $lang_module, $module_config, $module_info, $op, $array_config, $array_displays;
    ...
    if ($array_config['homedata']) {
        ...
        foreach ($array_viewtype as $index => $value) {
            $value['active'] = $index == $viewtype ? 'selected' : '';
            $value['index'] = $index;
 ...
 *
 */
function nv_theme_market_viewcat($array_data, $array_subcat_data, $viewtype, $page, $sort = 0)
{
    global $global_config, $module_name, $module_file, $lang_module, $module_config, $module_info, $op, $array_market_cat, $catid, $array_config, $array_displays;
        ...
        foreach ($array_data as $data) {
            ...
            if (!empty($data['custom_field'])) {
                foreach ($data['custom_field'] as $field) {
                    if (!empty($field['value']) and in_array(2, $field['show_locations'])) {
...
//     var_dump($content_comment);die;
    ...
    if (!empty($rows_other)) {
        if (nv_function_exists('nv_theme_market_' . $array_config['style_default'])) {
            $xtpl->assign('OTHER', call_user_func('nv_theme_market_' . $array_config['style_default'], $rows_other));
...
function nv_theme_market_search($array_data, $is_search, $viewtype, $page, $array_json)
{
    ...
    if ($is_search) {
        if (!empty($array_data)) {
            ...
            if ($array_json) {
                $xtpl->parse('main.result.maps.maps_' . $array_config['map_position']);
