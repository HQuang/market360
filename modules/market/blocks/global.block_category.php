...
if (! nv_function_exists('nv_market_category')) {

    function nv_block_config_market_category($module, $data_block, $lang_block)
    {
        ...
        foreach ($list as $l) {
            ...
            $html_input .= '<input type="hidden" id="config_catid_' . $l['id'] . '" value="' . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module . '&amp;' . NV_OP_VARIABLE . '=' . $l['alias'] . '" />';
            $html .= '<option value="' . $l['catid'] . '" ' . (($data_block['catid'] == $l['id']) ? ' selected="selected"' : '') . '>' . $xtitle_i . $l['title'] . '</option>';
        }
        $html .= '</select>';
        $html .= $html_input;
    ...
    function nv_block_config_market_category_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_market_category($block_config)
    {
        global $module_array_cat, $module_info, $lang_module, $global_config;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/market/block_category.tpl')) {
            $block_theme = $global_config['module_theme'];
        } else {
            $block_theme = 'default';
        }

        $xtpl = new XTemplate('block_category.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/market');
        ...
        if (! empty($module_array_cat)) {
            ...
            foreach ($module_array_cat as $cat) {
                if ($block_config['catid'] == 0 and $cat['parentid'] == 0 or ($block_config['catid'] > 0 and $cat['parentid'] == $block_config['catid'])) {
                    ...
                    if (! empty($cat['subid'])) {
                        $xtpl->assign('SUBCAT', nv_market_sub_category($cat['subid'], $title_length, $block_theme));
    ...
    function nv_market_sub_category($list_sub, $title_length, $block_theme)
    {
        ...
        if (empty($list_sub)) {
            ...
            foreach ($list as $catid) {
                ...
                if (! empty($subcat['subid'])) {
                    $xtpl->assign('SUB', nv_market_sub_category($subcat['subid'], $title_length, $block_theme));
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_market_category($block_config);
    }
}
