...
if (isset($site_mods['feedback']) and isset($module_config[$module_name]['activecomm'])) {
    ...
    // check allow comemnt
    $allowed = $module_config[$module_name]['groupcomment']; // tuy vào module để lấy cấu hình. Nếu là module news thì có cấu hình theo bài viết
    if ($allowed == '-1') {
        $allowed = $rows['groupcomment'];
    }
        ...
        if (!empty($data = nv_market_data($_row, $module_name))) {
            ...
            if (!isset($array_custom_field_title[$_row['catid']])) {
                if ($array_market_cat[$data['catid']]['form'] != '') {
                    ...
                    if ($idtemplate) {
                        $result2 = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field ORDER BY weight');
                        while ($row_field = $result2->fetch()) {
                            ...
                            if (in_array($idtemplate, $listtemplate)) {
                                ...
                                if (!empty($row_field['field_choices'])) {
                                    ...
                                    $weight = 0;
                                    while (list ($key, $val) = $result3->fetch(3)) {
...
if (!empty($array_field_config)) {
    $result = $db->query("SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_info WHERE rowid IN (" . implode(',', array_keys($rows_other)) . ")");

    while ($custom_fields = $result->fetch()) {
        foreach ($array_field_config as $row) {
            $row['show_locations'] = explode(',', $row['show_locations']);
            if (in_array(6, $row['show_locations'])) {
                ...
                $rows_other[$custom_fields['rowid']]['custom_field'][$row['field']] = $row;
            }
        }
    }
}
...
// thay thế h1 thành h2 trong nội dung
$rows['content'] = preg_replace('#<h1([^>]*)>(.*)</h1>#m','<h2$1>$2</h2>', $rows['content']);
