...
if ($idtemplate) {
    ...
    while ($_row = $result->fetch()) {
        $listtemplate = explode(',', $_row['listtemplate']);
        if (!in_array($idtemplate, $listtemplate)) continue;
        ...
        if ($value != '') {
            if ($_row['field_type'] == 'number') {
                ...
                if (!preg_match($pattern, $value)) {
                    nv_jsonOutput(array(
                        'error' => 1,
                        'msg' => sprintf($lang_module['field_match_type_error'], $_row['title'])
                    ));
                } else {
                    ...
                    if ($value < $_row['min_length'] or $value > $_row['max_length']) {
                        nv_jsonOutput(array(
                            'error' => 1,
                            'msg' => sprintf($lang_module['field_min_max_value'], $_row['title'], $_row['min_length'], $_row['max_length'])
                        ));
                    }
                }
                ...
                if (preg_match("/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/", $value, $m)) {
                    ...
                    if ($value < $_row['min_length'] or $value > $_row['max_length']) {
                        nv_jsonOutput(array(
                            'error' => 1,
                            'msg' => sprintf($lang_module['field_min_max_value'], $_row['title'], date('d/m/Y', $_row['min_length']), date('d/m/Y', $_row['max_length']))
                        ));
                    }
                } else {
                    nv_jsonOutput(array(
                ...
                if ($_row['match_type'] == 'alphanumeric') {
                    ...
                    if (function_exists($_row['func_callback'])) {
                        ...
                        nv_jsonOutput(array(
                            'error' => 1,
                            'msg' => "error function not exists " . $_row['func_callback']
                        ));
                    }
                } else {
