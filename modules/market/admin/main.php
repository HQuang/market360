...
if ($nv_Request->isset_request('change_queue', 'post')) {
    $listall = $nv_Request->get_title('listall', 'post', '');
    $array_id = explode(',', $listall);

    if (! empty($array_id)) {
        foreach ($array_id as $id) {
            $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET is_queue=0 WHERE id=' . $id);
        }
        $nv_Cache->delMod($module_name);
        die('OK');
    }
...
$array_action['delete_list_id'] = $lang_module['delete'];
