...
if ($nv_Request->isset_request('savesetting', 'post')) {
    ...
    $data['map_position'] = $nv_Request->get_title('map_position', 'post', 'top');
...
$array_map_position = array(
    'top' => $lang_module['config_map_top'],
    'right' => $lang_module['config_map_right'],
    'bottom' => $lang_module['config_map_bottom'],
    'left' => $lang_module['config_map_left'],
);
foreach ($array_map_position as $index => $value) {
    $sl = $index == $array_config['map_position'] ? 'selected="selected"' : '';

    $xtpl->assign('MAP_POSITION', array(
        'index' => $index,
        'value' => $value,
        'selected' => $sl
    ));
    $xtpl->parse('main.map_position');
}
...
if (!empty($array_config['grouppostconfig'])) {
    ...
    foreach ($array_config['grouppostconfig'] as $groupid => $groupconfig) {
        ...
        $groupconfig['ck_allow_link'] = (isset($groupconfig['allow_link']) and $groupconfig['allow_link']) ? 'checked="checked"' : '';
