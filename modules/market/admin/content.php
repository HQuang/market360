...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (!$array_config['allow_auto_code'] and empty($row['code'])) {
        nv_jsonOutput(array(
            'error' => 1,
            'msg' => $lang_module['error_required_code'],
            'input' => 'code'
        ));
    } elseif (!preg_match('/^[a-zA-Z0-9]*$/', $row['code'])) {
        nv_jsonOutput(array(
            'error' => 1,
            'msg' => $lang_module['error_vaild_code'],
            'input' => 'code'
        ));
        ...
        nv_jsonOutput(array(
            'error' => 1,
            'msg' => $lang_module['error_duplicate_code'],
            'input' => 'code'
        ));
    ...
    try {
        ...
        if ($new_id > 0) {
            ...
            if (empty($row['id'])) {
                ...
                $wallet->update($fee, 'VND', $admin_info['userid'], $mess_);
                ...
                if (empty($row['is_queue'])) {
                    $wallet->update($fee, 'VND', $row['userid'], $mess_);
                }
