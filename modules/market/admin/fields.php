...
$sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_template WHERE status=1';
...
$result = $db->query($sql);$array_template = array();
...
if ($nv_Request->isset_request('changeweight', 'post')) {
    ...
    $query = 'SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $fid;
    $numrows = $db->query($query)->fetchColumn();
    if ($numrows != 1) {
        die('NO');
    }

    $query = 'SELECT fid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid!=' . $fid . ' ORDER BY weight ASC';
    ...
    while ($row = $result->fetch()) {
        ...
        $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_field SET weight=' . $weight . ' WHERE fid=' . $row['fid'];
        $db->query($sql);
    }
    $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_field SET weight=' . $new_vid . ' WHERE fid=' . $fid;
    $db->query($sql);
    nv_htmlOutput('OK');
}
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $array_showlocation = array_unique($nv_Request->get_typed_array('show_locations', 'post', 'int', array()));

    if (!empty($templateids)) {
        $dataform['show_locations'] = implode(",", $array_showlocation);
    } else {
        $dataform['show_locations'] = '';
    }
    ...
    $save = 0;
    $dataform_old = $language = array();
    if ($dataform['fid']) {
        $dataform_old = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $dataform['fid'])->fetch();
        $dataform['field_type'] = $dataform_old['field_type'];
        if (!empty($dataform['language'])) {
            $language = unserialize($dataform['language']);
        }
        $dataform['field'] = $dataform['fieldid'] = nv_strtolower(nv_substr($nv_Request->get_title('fieldid', 'post', '', 0, $preg_replace), 0, 50));
    } else {
        $dataform['field'] = nv_strtolower(nv_substr($nv_Request->get_title('field', 'post', '', 0, $validatefield), 0, 50));
        ...
        if (in_array($dataform['field'], $field_not_allow)) {
            ...
            $stmt = $db->prepare('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE field= :field');
    ...
    if (empty($error)) {
        ...
        if (empty($dataform['fid'])) {
            ...
            if ($dataform['max_length'] <= 4294967296 and !empty($dataform['field']) and !empty($dataform['title'])) {
                $weight = $db->query('SELECT MAX(weight) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field')->fetchColumn();
                ...
                if ($dataform['fid']) {
                    $db->query("ALTER TABLE " . NV_PREFIXLANG . "_" . $module_data . "_info ADD " . $dataform['field'] . " " . $type_date);
            ...
            try {
                ...
                if ($save) {
                    $db->query("ALTER TABLE " . NV_PREFIXLANG . "_" . $module_data . "_info CHANGE " . $dataform_old['field'] . " " . $dataform_old['field'] . " " . $type_date);
                    nv_create_form_file($templateids);
                }
                nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&rand=' . nv_genpass());
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    list ($fid, $listtemplate, $field, $weight) = $db->query('SELECT fid,listtemplate, field, weight FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $fid)->fetch(3);
    if ($fid and !empty($field)) {
        $query1 = 'DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $fid;
        if ($db->query($query1)) {
            $db->query('ALTER TABLE ' . NV_PREFIXLANG . '_' . $module_data . '_info DROP ' . $field);

            // sắp xếp lại weight
            $query = 'SELECT fid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE weight > ' . $weight . ' ORDER BY weight ASC';
            $result = $db->query($query);
            while ($row = $result->fetch()) {
                $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_field SET weight=' . $weight . ' WHERE fid=' . $row['fid']);
                ++$weight;
            }
            $del_array = array(
                $listtemplate
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()
        ->select('COUNT(*)')
        ->from('' . NV_PREFIXLANG . '_' . $module_data . '_field');
...
if (!isset($dataform)) {
    if ($fid) {
        $dataform = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field WHERE fid=' . $fid)->fetch();
...
if (empty($fid)) {
    ...
    $f_count = $db->query('SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_field where fid = ' . $fid)->fetchColumn();
    if ($f_count < 1) {
...
$array_show_locations = array(
    1 => $lang_module['field_show_block'],
    2 => $lang_module['field_show_viewlist'],
    3 => $lang_module['field_show_viewgrid'],
...
foreach ($array_show_locations as $sl_key => $sl_val) {
    $sl_val = array(
        'id' => $sl_key,
        'title' => $sl_val,
        'checked' => (in_array($sl_key, $explode_show_location)) ? ' checked="checked"' : ''
    );
    $xtpl->assign('SL', $sl_val);
    $xtpl->parse('main.load.field_show_locations');
}
