...
$table_name = '' . NV_PREFIXLANG . '_' . $module_data . '_template';
...
if (!empty($savecat)) {
    ...
    $count = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_template WHERE alias=' . $db->quote($data['alias']) . ' AND id!=' . $data['id'])
        ->fetchColumn();

    if ($count > 0) {
        $_tem_id = $db->query('SELECT MAX(id) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_template WHERE alias=' . $db->quote($data['alias']))
            ->fetchColumn();
    ...
    if (empty($data['title'])) {
        ...
        if ($data['id'] == 0) {
            ...
            $sql = "INSERT INTO " . $table_name . " (status, title, alias, weight) VALUES (1, " . $db->quote($data['title']) . ", " . $db->quote($data['alias']) . ", " . $weight . ")";
            ...
            $stmt = $db->prepare("UPDATE " . $table_name . " SET title= :title WHERE id =" . $data['id']);
...
$result = $db->query("SELECT id, title, alias, status, weight FROM " . $table_name . " ORDER BY weight ASC");
$num = $result->rowCount();
...
while (list ($id, $title, $alias, $status, $weight) = $result->fetch(3)) {
    ...
    $xtpl->assign('link_field', NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=fields&templateid=" . $id);
    $xtpl->assign('id', $id);
