...
$_sql = 'SELECT id, icon, title, note, dfault FROM ' . NV_PREFIXLANG . '_' . $module_data . '_widget where status=1';
$array_wid = $nv_Cache->db($_sql, 'id', $module_name);
...
{
    global $db, $lang_module, $lang_global, $module_name, $module_data, $op, $module_file, $my_head;
        ...
        while ($row = $result->fetch()) {
            ...
            if (in_array($idtemplate, $listtemplate)) {
                ...
                if ($row['field_type'] == 'date') {
                    ...
                    if (!defined('NV_EDITOR') && !nv_function_exists('nv_aleditor') and file_exists(NV_ROOTDIR . '/' . NV_EDITORSDIR . '/ckeditor/ckeditor.js')) {
                    ...
                    if (defined('NV_EDITOR')) {
                        ...
                        $edits = nv_aleditor('custom[' . $row['fid'] . ']', $row['class'][0], $row['class'][1], $array_custom[$row['field']]);
                    ...
                    $number = 0;
                    $array_custom[$row['field']] = explode(',', $array_custom[$row['field']]);
                    foreach ($row['field_choices'] as $key => $value) {
                        $xtpl->assign('OPTION', array(
                            'id' => $row['fid'] . '_' . $number++,
                            'key' => $key,
                            'checked' => in_array($key, $array_custom[$row['field']]) ? ' checked="checked"' : '',
                            'title' => $value
                        ));
                    ...
                    foreach ($row['field_choices'] as $key => $value) {
                        $xtpl->assign('OPTION', array(
                            'key' => $key,
                            'selected' => in_array($key, $array_custom[$row['field']]) ? ' selected="selected"' : '',
                            'title' => $value
                        ));
