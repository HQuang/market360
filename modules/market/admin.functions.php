...
function nv_get_data_type($dataform)
{
    ...
    if ($dataform['field_type'] == 'number') {
        ...
        $type_date = "TINYTEXT NOT NULL DEFAULT ''";
    ...
    foreach ($array_template_id as $templateids_i) {
        ...
        foreach ($array_views as $key => $column) {
            ...
            if ($column['field_type'] == 'textarea') {
                $content_2 .= "<textarea class=\"form-control\" style=\"width: 98%; height:100px;\" cols=\"75\" rows=\"5\" name=\"custom[" . $key . "]\">{ROW." . $column['field'] . "}</textarea>";
                ...
                $content_2 .= "\t\t\t\t";
            } elseif ($column['field_type'] == 'radio') {
                $type_html = ($column['field_type'] == 'radio') ? 'radio' : 'checkbox';
                $content_2 .= "\n\t\t\t\t\t<!-- BEGIN: " . $type_html . "_" . $key . " -->\n";
                $content_2 .= "\t\t\t\t\t<label><input class=\"form-control\" type=\"" . $type_html . "\" name=\"custom[" . $key . "]\" value=\"{OPTION.key}\" {OPTION.checked}";
...
function nv_crawler_groups_delete($id)
{
    ...
    if ($weight > 0) {
        ...
        $items_info = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_crawler_items WHERE groups_id=' . $id)->fetch();
        if (!empty($items_info)) {
            // Xóa nguồn tin
            $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_crawler_items  WHERE groups_id=' . $id);

            // Xóa tin tức
            $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_crawler_rows  WHERE items_id=' . $items_info['id']);
        }
    }
}

function nv_crawler_delete_rows($id)
{
    global $db, $module_data;
